...
::ROTATE 180
::ffmpeg -i %1    -qscale:v 2 -vf "select='eq(pict_type,PICT_TYPE_I)',lut3d=BESTMOYEN.CUBE,hflip, vflip"    -vsync vfr   %~n1\%~n1_%%04d.jpg
:: SANS ROTATION 180

::ffmpeg -i %1    -qscale:v 2 -vf "select='eq(pict_type,PICT_TYPE_I)',lut3d=BESTMOYEN.CUBE"    -vsync vfr   %~n1\%~n1_%%04d.jpg


for %%1 in ("*.mp4") do md %%~n1 | ffmpeg -i %%1 -qscale:v 2 -vf "select='eq(pict_type,PICT_TYPE_I)',lut3d=BESTMOYEN.CUBE"  -vsync vfr %%~n1\%%~n1_%%04d.jpg
pause
